...
const dataApplier = function (formConfigObject) {
    ...
    // Form-Config Apply
    appliedObject.formConfig = Object.assign(
        {},
        FORM_DEFAULT_DATA,
        formConfigObject.formConfig
    );

    // Section(s) Apply
    for (let [sectionId, sectionObject] of Object.entries(
        formConfigObject.sections
    )) {
        appliedObject.sections[sectionId] = baseObjectExtend(
    ...
    for (let [rowId, rowObject] of Object.entries(formConfigObject.rows)) {
        appliedObject.rows[rowId] = baseObjectExtend(
            ROW_DEFAULT_DATA,
            rowObject
        );
    }
    ...
    for (let [controlId, controlObject] of Object.entries(
        ...
        let baseDefaultConfig = baseObjectExtend(
            CONTROL_DEFAULT_DATA,
            baseConfigOfType
        );

        // add to base
        appliedObject.controls[controlId] = Object.assign(
            baseDefaultConfig,
            controlObject
        );
    }
